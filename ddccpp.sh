...
WIDTH=60
SEL_HEIGHT=12
BACKTITLE="DDCCPP ${VER} - Distribution by littleboi.com"
TITLE=""
MENU="Detected as ${ESOS^^}"
...
case $SEL in
	1)
		...
		if [ $ESOS != "arkos" ]; then
			...
			if [ -f "$THEME_PATH/es-theme-freeplay/_inc/fonts/Erbaum-Book.otf" ]; then
				cp -f ./$TEMP/NotoSansKR-Light.otf $THEME_PATH/es-theme-freeplay/_inc/fonts/Erbaum-Book.otf
			fi
			if [ -f "$THEME_PATH/es-theme-freeplay/_inc/fonts/Erbaum-Bold.otf" ]; then
				cp -f ./$TEMP/NotoSansKR-Light.otf $THEME_PATH/es-theme-freeplay/_inc/fonts/Erbaum-Bold.otf
			fi
			...
			if [ -f "$THEME_PATH/es-theme-gbz35_mod/_inc/fonts/Roboto-Light.ttf" ]; then
				cp -f ./$TEMP/opensans_hebrew_condensed_light.ttf $THEME_PATH/es-theme-gbz35_mod/_inc/fonts/Roboto-Light.ttf
			fi
			if [ -f "$THEME_PATH/es-theme-gbz35_mod/_inc/fonts/RobotoCondensed-Regular.ttf" ]; then
				cp -f ./$TEMP/opensans_hebrew_condensed_regular.ttf $THEME_PATH/es-theme-gbz35_mod/_inc/fonts/RobotoCondensed-Regular.ttf
			fi
			...
			if [ -f "$THEME_PATH/es-theme-nes-box/_inc/fonts/sans-black-cond-italic.ttf" ]; then
				cp -f ./$TEMP/opensans_hebrew_condensed_regular.ttf $THEME_PATH/es-theme-nes-box/_inc/fonts/sans-black-cond-italic.ttf
			fi
			...
			if [ -f "$THEME_PATH/es-theme-switch/_art/Acre.otf" ]; then
				cp -f ./$TEMP/NotoSansKR-Light.otf $THEME_PATH/es-theme-switch/_art/Acre.otf
			fi
		fi
		...
		if [ $HOSTNAME == "BATOCERA" ]; then
			...
			if cat /home/ark/.emulationstation/es_settings.cfg | grep Language >/dev/null; then
				sed -i 's/name="Language" value=.*"/name="Language" value="ko"/g' /home/ark/.emulationstation/es_settings.cfg
			else
				sed -i 's/<string name="LogLevel"/<string name="Language" value="ko" \/>\n<string name="LogLevel"/g' /home/ark/.emulationstation/es_settings.cfg
			fi
		fi
...
mkdir -m 777 -p ./$TEMP/mam
mkdir -m 777 -p ./$TEMP/fbn
...
echo -e "."
		...
		do
			...
			mkdir -m 777 -p "$RA_CFG/$TCORE"
			mkdir -m 777 -p "$RA_RMP/$TCORE"
